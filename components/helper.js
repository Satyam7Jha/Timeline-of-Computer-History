window.addEventListener("load", () => {
  ...
  timeLinePage.addEventListener("click", () =>
    window.open("./components/TimeLine.html", "_self")
  );
  ...
  window.onmousemove = (e) => {
    ...
    const mouseDelta =
      parseFloat(track.getAttribute("mouseDownAt")) - e.clientX;
    let maxDelta = window.innerWidth / 2;
    const percentage = (mouseDelta / maxDelta) * -100;
    nextPercentage =
      percentage + parseFloat(track.getAttribute("prevPercentage"));
    ...
    for (let i = 0; i < imageList.length; i++) {
      ...
      imageList[i].animate(
        {
          objectPosition: `${
            100 - Math.abs(track.getAttribute("percentage"))
          }% 50%`,
        },
        { duration: 1200, fill: "forwards" }
      );
  ...
  const handleSlideChage = (direction) => {
    ...
    if (slide < 0) slide = slideData.length - 1;
    if (slide > slideData.length - 1) slide = 0;
    modalImage.setAttribute("src", slideData[slide]["src"]);
